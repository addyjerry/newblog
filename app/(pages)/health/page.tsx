...
const Page = async () => {
  const healthNewsData = await getHealth();
  const healthNews = healthNewsData.data;

  console.log("healthNews array:", healthNews);

  // Log first item to see imageUrl structure
  if (healthNews.length > 0) {
    console.log("First news item:", JSON.stringify(healthNews[0], null, 2));
    console.log("imageUrl structure:", healthNews[0].imageUrl);
  }
  ...
  if (!healthNews || healthNews.length === 0) {
    return (
      <main className="items-center w-full max-w-[1500px] place-self-center my-10">
        ...
        <p>No Health news articles found.</p>
      </main>
    );
  }
  ...
  return (
    <main className="items-center w-full max-w-[1500px] place-self-center my-10">
      <h1 className="text-2xl font-bold mb-4">
        Health News ({healthNews.length} articles)
      </h1>
