...
import { getCulture } from "@/lib/api";
import Image from "next/image";
import Link from "next/link";

const Page = async () => {
  const cultureNewsData = await getCulture();
  const cultureNews = cultureNewsData.data;

  console.log("cultureNews array:", cultureNews);

  // Log first item to see imageUrl structure
  if (cultureNews.length > 0) {
    console.log("First news item:", JSON.stringify(cultureNews[0], null, 2));
    console.log("imageUrl structure:", cultureNews[0].imageUrl);
  }

  if (!cultureNews || cultureNews.length === 0) {
    return (
      <main className="items-center w-full max-w-[1500px] place-self-center my-10">
        <h1 className="text-2xl font-bold mb-4">culture News</h1>
        <p>No culture news articles found.</p>
      </main>
    );
  }
  ...
  return (
    <main className="items-center w-full max-w-[1500px] place-self-center my-10">
      <h1 className="text-2xl font-bold mb-4">
        culture News ({cultureNews.length} articles)
      </h1>
