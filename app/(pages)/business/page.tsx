...
const Page = async () => {
  const businessNewsData = await getBusiness();
  const businessNews = businessNewsData.data;

  console.log("businessNews array:", businessNews);

  // Log first item to see imageUrl structure
  if (businessNews.length > 0) {
    console.log("First news item:", JSON.stringify(businessNews[0], null, 2));
    console.log("imageUrl structure:", businessNews[0].imageUrl);
  }
  ...
  if (!businessNews || businessNews.length === 0) {
    return (
      <main className="items-center w-full max-w-[1500px] place-self-center my-10">
        ...
        <p>No Business news articles found.</p>
      </main>
    );
  }
  ...
  return (
    <main className="items-center w-full max-w-[1500px] place-self-center my-10">
      <h1 className="text-2xl font-bold mb-4">
        Business News ({businessNews.length} articles)
      </h1>
