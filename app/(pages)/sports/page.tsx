...
const Page = async () => {
  const sportsNewsDataData = await getSports();
  const sportsNewsData = sportsNewsDataData.data;

  console.log("sportsNewsData array:", sportsNewsData);

  // Log first item to see imageUrl structure
  if (sportsNewsData.length > 0) {
    console.log("First news item:", JSON.stringify(sportsNewsData[0], null, 2));
    console.log("imageUrl structure:", sportsNewsData[0].imageUrl);
  }

  if (!sportsNewsData || sportsNewsData.length === 0) {
    return (
      <main className="items-center w-full max-w-[1500px] place-self-center my-10">
        ...
        <p>No Sports news articles found.</p>
      </main>
    );
  }
  ...
  return (
    <main className="items-center w-full max-w-[1500px] place-self-center my-10">
      <h1 className="text-2xl font-bold mb-4">
        Sports News ({sportsNewsData.length} articles)
      </h1>
