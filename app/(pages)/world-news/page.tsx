...
const Page = async () => {
  const worldNewsData = await getWorldNews();
  const worldNews = worldNewsData.data;

  console.log("worldNews array:", worldNews);

  // Log first item to see imageUrl structure
  if (worldNews.length > 0) {
    console.log("First news item:", JSON.stringify(worldNews[0], null, 2));
    console.log("imageUrl structure:", worldNews[0].imageUrl);
  }

  if (!worldNews || worldNews.length === 0) {
    return (
      <main className="items-center w-full max-w-[1500px] place-self-center my-10">
        ...
        <p>No world news articles found.</p>
      </main>
    );
  }
  ...
  return (
    <main className="items-center w-full max-w-[1500px] place-self-center my-10">
      <h1 className="text-2xl font-bold mb-4">
        World News ({worldNews.length} articles)
      </h1>
